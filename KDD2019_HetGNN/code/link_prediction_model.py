...
def load_data(data_file_name, n_features, n_samples):
    with open(data_file_name) as f:
        ...
        for i, d in enumerate(data_file):
            data[i] = numpy.asarray(d[:], dtype=numpy.float64)    
        f.close  
...
def model(train_num, test_num):
	...
	train_features = train_data.astype(numpy.float64)[:,3:-1]
	train_target=train_data.astype(numpy.float64)[:,2]
	...
	test_features = test_data.astype(numpy.float64)[:,3:-1]
	test_target = test_data.astype(numpy.float64)[:,2]
