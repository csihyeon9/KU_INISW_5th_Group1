...
class KeywordRecommender:
    ...
    def calculate_similarity(self, input_keyword, top_k=5):
        if input_keyword not in self.data['keywords']:
            raise ValueError(f"Keyword '{input_keyword}' not found in data.")
        ...
        similarities = [
            (keyword, 1 - cosine(input_embedding, features[i]))
            for i, keyword in enumerate(keywords) if keyword != input_keyword
        ]
