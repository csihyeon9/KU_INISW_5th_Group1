...
def save_hgnn_data(output_path, feature_matrix, H, W, L, keyword_to_idx, labels):
    ...
    torch.save({
        'X': torch.tensor(feature_matrix, dtype=torch.float32), # 노드 특징 행렬    
        'H': torch.tensor(H, dtype=torch.float32),              # 인시던스 행렬
        'W': torch.tensor(W, dtype=torch.float32),              # 엣지 가중치 행렬
        'L': L,                                                 # 정규화된 라플라시안 행렬
        'keyword_to_idx': keyword_to_idx,                       # 레이블
        'labels': labels
    }, output_path)
