...
def load_json_data(file_path):
    ...
    # filtering
    filtered_data = [doc for doc in data if doc.get('keywords') and len(doc['keywords']) > 0]
    
    # if len(filtered_data) < len(data):
    #     print(f"Filtered out {len(data) - len(filtered_data)} datas with no keywords.")
...
def create_keyword_embeddings(documents, embedding_size=128):
    """Create keyword embeddings using Word2Vec"""
    all_keywords = [doc['keywords'] for doc in documents]
    w2v_model = Word2Vec(all_keywords, vector_size=embedding_size, 
                        window=5, min_count=1, workers=4)
    return w2v_model

def preprocess_documents(documents, keyword_model, embedding_size=128):
    """Create document features from keyword embeddings"""
    n_docs = len(documents)
    features = np.zeros((n_docs, embedding_size))
    
    for i, doc in enumerate(documents):
        keyword_vectors = [keyword_model.wv[word] for word in doc['keywords'] 
                         if word in keyword_model.wv]
        if keyword_vectors:
            features[i] = np.mean(keyword_vectors, axis=0)
