...
class HyperConv(Module):
    ...
    def forward(self, adjacency, embedding):
        ...
        # 리스트를 텐서로 변환하고 detach 처리
        final_tensor = torch.stack(final)
        item_embeddings = final_tensor.detach().cpu().sum(0) / (self.layers + 1)
...
class DHCN(Module):
    def __init__(self, adjacency, n_node, lr, layers, l2, beta, dataset, emb_size=100, batch_size=100):
        ...
        # Convert csr_matrix to coo_matrix to access .row and .col
        coo_adjacency = adjacency.tocoo()
        
        values = coo_adjacency.data
        indices = np.vstack((coo_adjacency.row, coo_adjacency.col))

        if dataset == 'Nowplaying':
            index_filter = (values < 0.05).nonzero()
            values = np.delete(values, index_filter)
            indices1 = np.delete(indices[0], index_filter)
            indices2 = np.delete(indices[1], index_filter)
        ...
        # Use torch.sparse_coo_tensor to create sparse tensor
    ...
    def generate_sess_emb(self,item_embedding, session_item, session_len, reversed_sess_item, mask):
        ...
        seq_h = torch.zeros(self.batch_size, reversed_sess_item.shape[1], self.emb_size, device=device)
    ...
    def SSL(self, sess_emb_hgnn, sess_emb_lgcn):
        ...
        one = torch.ones(neg1.shape[0], device=sess_emb_hgnn.device)
...
def train_test(model, train_data, test_data):
    ...
    for i in slices:
        ...
        targets = torch.clamp(targets, min=0)  # Ensure targets are within valid range
